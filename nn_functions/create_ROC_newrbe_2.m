function [net, accuracy_n, neurons, best_n] = create_ROC_newrbe_2(input, targets, hn_min, hn_max, hn_step, trains)
    %% Create the waitbar
    ...
    [~,min_distance] = min_spread(input);
    [~,max_distance] = max_spread(input);
    s_min = min_distance;       % Minimum spread to use
    s_max = max_distance;       % Maximum spread to use
    ...
    input_train = [input(1:train_index*4,:); input(test_index*4+1:valid_index*4,:)];
    x_test = x(:,train_index*4+1:test_index*4);
    targets_train = [targets(1:train_index*4, :); targets(test_index*4+1:valid_index*4,:)];
    ...
    % Average MSE of the spreads, average R-value of the spreads and spread values
    ...
    for hn = hn_min : hn_step : hn_max
        ...
        for i = 1 : trains
            % Compute randomly "hn" centres and the relative spread
            [centres, out_centres, spread(i)] = cluster_spread(input_train, targets_train, hn, i); 

            % Create a RBF: "evalc" doesn't write on the command window all steps performed
            [~,net] = evalc('newrbe(centres,out_centres,spread(i))');
            ...
            yind = vec2ind(y);
            for j = 1 : n_classes
                accuracy_s(i, j) = sum(yind(tind == j) == j)/sum(tind == j);
            end
            accuracy_s(i, n_classes + 1) = sum(tind == yind)/numel(tind);
            results_s(:,:,i) = y;
            nets_s{i} = net;
        end
